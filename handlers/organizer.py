...
async def my_conferences(message: types.Message):
    ...
    async with AsyncSessionLocal() as session:
        ...
        for conf in conferences:
            ...
            builder.row(
                InlineKeyboardButton(text=f"Удалить конференцию", callback_data=f"delete_conf_{conf.id}"))
            builder.row(InlineKeyboardButton(text=f"📢 Рассылка участникам конференции", callback_data=f"broadcast_{conf.id}"))
            builder.row(InlineKeyboardButton(text=f"📊 Экспорт участников конференции", callback_data=f"export_conf_{conf.id}"))
...
async def do_delete(callback: types.CallbackQuery):
    ...
    try:
        await callback.message.edit_text(
            f"Конференция <b>{conf.name}</b> успешно удалена."
        )
    except Exception:
...
# Валидация даты (оставлена из старого кода)
